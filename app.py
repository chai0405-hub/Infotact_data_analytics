...
def load_data():
    ...
    if cleaned_file is None:
        st.error(
            "Missing cleaned dataset. Please keep one of these files inside the data folder:\n\n"
    ...
    for col in ["ad_spend", "conversion_value", "is_conversion"]:
        if col in cleaned_df.columns:
            cleaned_df[col] = pd.to_numeric(cleaned_df[col], errors="coerce").fillna(0)

    if attribution_df is not None:
        for col in ["attributed_conversions", "attributed_revenue"]:
            if col in attribution_df.columns:
                attribution_df[col] = pd.to_numeric(attribution_df[col], errors="coerce").fillna(0)
...
if missing_summary_columns:
    st.error(f"Missing columns in attribution_channel_summary.csv: {missing_summary_columns}")
    st.stop()
...
default_model_index = models.index("Linear") if "Linear" in models else 0
...
st.sidebar.markdown("---")
st.sidebar.subheader("Select Channels")

channels = sorted(summary_df["channel"].dropna().unique())

select_all_channels = st.sidebar.checkbox("Select All Channels", value=True)

selected_channels = []

if select_all_channels:
    selected_channels = channels
    st.sidebar.success("All channels selected")
else:
    st.sidebar.caption("Tick the channels you want to analyze:")
    for channel in channels:
        if st.sidebar.checkbox(channel, value=True, key=f"channel_{channel}"):
...
with chart_col1:
    ...
    revenue_chart.update_layout(height=430, xaxis_title="Attributed Revenue", yaxis_title="Channel")
...
with chart_col2:
    ...
    roas_chart.update_layout(height=430, xaxis_title="ROAS", yaxis_title="Channel")
...
with chart_col3:
    ...
    cac_chart.update_layout(height=430, xaxis_title="CAC", yaxis_title="Channel")
...
with chart_col4:
    ...
    conversion_chart.update_layout(height=430, xaxis_title="Attributed Conversions", yaxis_title="Channel")
...
roi_scatter.update_layout(height=500, xaxis_title="Total Spend", yaxis_title="Attributed Revenue")
st.plotly_chart(roi_scatter, use_container_width=True)

st.caption("Interpretation: Channels with higher revenue and lower spend are generally more efficient.")

st.divider()

# ------------------------------------------------------------
# 3. Funnel Analysis and Drop-off Rate
# ------------------------------------------------------------
...
if "funnel_stage" in cleaned_df.columns and "event_id" in cleaned_df.columns:
    ...
    # Stage change can be positive or negative.
    funnel_df["stage_change"] = (
        funnel_df["touchpoints"] - funnel_df["previous_stage_touchpoints"]
    ).fillna(0)

    # Drop-off should not be negative. If touchpoints increase, drop-off is 0.
    funnel_df["dropoff_count"] = (
        funnel_df["previous_stage_touchpoints"] - funnel_df["touchpoints"]
    )
    funnel_df["dropoff_count"] = funnel_df["dropoff_count"].clip(lower=0).fillna(0)

    funnel_df["dropoff_rate_percent"] = np.where(
        funnel_df["previous_stage_touchpoints"].fillna(0) == 0,
        0,
    ...
    display_funnel["stage_change"] = display_funnel["stage_change"].round(2)
    display_funnel["dropoff_count"] = display_funnel["dropoff_count"].round(2)
    ...
    st.dataframe(
        display_funnel[
            ["funnel_stage", "touchpoints", "stage_change", "dropoff_count", "dropoff_rate_percent"]
        ],
...
st.caption("This section helps a performance marketer drill down from channel-level performance to campaign-level performance.")
...
if attribution_df is not None and required_attribution_columns.issubset(attribution_df.columns):
    ...
    fallback_required_columns = {"channel", "campaign", "conversion_value", "is_conversion"}

    if fallback_required_columns.issubset(cleaned_df.columns):
        fallback_df = cleaned_df[cleaned_df["channel"].isin(selected_channels)].copy()

        campaign_summary = (
            fallback_df.groupby(["channel", "campaign"])
            .agg(
                attributed_conversions=("is_conversion", "sum"),
...
if not campaign_summary.empty:
    ...
    if {"channel", "campaign", "ad_spend"}.issubset(cleaned_df.columns):
        ...
        campaign_summary = campaign_summary.merge(campaign_spend, on=["channel", "campaign"], how="left")
    else:
    ...
    top_campaigns = campaign_summary.sort_values("attributed_revenue", ascending=False).head(15)
    ...
    campaign_chart.update_layout(height=550, xaxis_title="Attributed Revenue", yaxis_title="Campaign")
    ...
    campaign_table = campaign_summary[
        ["channel", "campaign", "attributed_conversions", "attributed_revenue", "total_spend", "roas", "cac"]
    ].copy()
    ...
    st.error(
        "Campaign-level data could not be generated. Please check that your dataset contains channel, campaign, ad_spend, conversion_value, and is_conversion columns."
    )

st.divider()

# ------------------------------------------------------------
# 5. Attribution Channel Summary Table
# ------------------------------------------------------------
...
top_revenue_channel = filtered_df.sort_values("attributed_revenue", ascending=False)["channel"].iloc[0]
top_roas_channel = filtered_df.sort_values("roas", ascending=False)["channel"].iloc[0]
lowest_cac_channel = filtered_df.sort_values("cac", ascending=True)["channel"].iloc[0]
