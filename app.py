...
# Version: Final Visible Campaign Drilldown
# ============================================================
...
# ------------------------------------------------------------
# Validate required summary columns
# ------------------------------------------------------------
...
st.caption("Interpretation: Channels in the upper-left area are more efficient because they generate higher revenue with lower spend.")
...
if "funnel_stage" in cleaned_df.columns and "event_id" in cleaned_df.columns:
    ...
    st.dataframe(
        display_funnel[["funnel_stage", "touchpoints", "dropoff_count", "dropoff_rate_percent"]],
...
st.caption("This section helps a performance marketer drill down from channel-level data to campaign-level performance.")
...
if attribution_df is not None and required_attribution_columns.issubset(attribution_df.columns):
    ...
    # Fallback method: build campaign view directly from cleaned dataset.
    # This keeps Section 4 visible even if attribution_model_output.csv is missing.
    st.warning(
        "data/attribution_model_output.csv was not found or has missing columns. "
        "Showing fallback campaign analysis from cleaned dataset."
    )

    if {"channel", "campaign", "event_id", "conversion_value", "is_conversion"}.issubset(cleaned_df.columns):
...
if not campaign_summary.empty:
    ...
    top_campaigns = (
        campaign_summary.sort_values("attributed_revenue", ascending=False)
        .head(15)
    )
...
st.success(
    "Dashboard includes attribution model toggle, ROI scatter plot, funnel drop-off analysis, campaign-level drilldown, and executive insights."
)
